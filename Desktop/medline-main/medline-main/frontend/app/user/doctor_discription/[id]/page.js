...
export default function DoctorDetailPage() {
  ...
  if (doctor && doctor.availability) {
    try {
      // Check if availability is already an object
      if (typeof doctor.availability === 'object') {
        parsedAvailability = doctor.availability;
      } else if (typeof doctor.availability === 'string') {
        // Try to parse as JSON, but handle non-JSON strings gracefully
        if (doctor.availability.trim().startsWith('{') || doctor.availability.trim().startsWith('[')) {
          parsedAvailability = JSON.parse(doctor.availability);
        } else {
          // Handle plain text availability like "monday to friday"
          console.log("Availability is plain text:", doctor.availability);
          parsedAvailability = { "General": [doctor.availability] };
        }
      }
    } catch (e) {
      ...
      parsedAvailability = { "General": [doctor.availability || "Not specified"] };
    }
  }
        ...
        <div className="max-w-4xl mx-auto apollo-card rounded-2xl p-8 mx-4 sm:mx-auto">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-grow md:w-2/3">
              <h2 className="text-2xl font-bold text-blue-600 border-b border-gray-200 pb-2 mb-4">About Dr. {doctor.name.split(' ').slice(1).join(' ')}</h2>
              <p className="text-gray-700 leading-relaxed">{doctor.description}</p>
              <div className="mt-8">
                <h2 className="text-2xl font-bold text-blue-600 border-b border-gray-200 pb-2 mb-4">Weekly Availability</h2>
                {Object.keys(parsedAvailability).length > 0 ? (
                  <div className="space-y-3">
                    {Object.entries(parsedAvailability).map(([day, times]) => (
                      <div key={day} className="grid grid-cols-3 gap-4 items-center">
                        <strong className="text-gray-800 col-span-1">{day}:</strong>
                        <div className="col-span-2 flex flex-wrap gap-2">
                          {Array.isArray(times) && times.map((time) => (
                            <span key={time} className="bg-blue-100 text-blue-700 text-sm font-medium px-3 py-1 rounded-full">{time}</span>
                          ))}
                        </div>
                      </div>
                    ))}
              ...
              <div className="mt-8">
                <button 
                  onClick={() => router.push(`/user/appointment?doctorId=${id}&doctorName=${encodeURIComponent(doctor.name)}`)}
                  className="w-full apollo-btn-primary font-bold py-3 px-6 rounded-lg transition-colors duration-300 shadow-md hover:shadow-lg"
                >
                  Make an Appointment
                </button>
              </div>
          ...
          <div className="mt-12 text-center">
            <Link href="/user/doctor_details" className="text-blue-600 hover:underline">
