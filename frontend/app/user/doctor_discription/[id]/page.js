        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex-shrink-0 text-center md:text-left md:w-1/3">
            <img
              ...
              alt={doctor.name}
              className="w-40 h-40 rounded-full object-cover border-4 border-gray-100 shadow-md mx-auto"
            />
            <h1 className="text-3xl font-bold mt-4 text-gray-800">{doctor.name}</h1>
            <p className="text-blue-600 font-semibold text-lg">{doctor.specialization}</p>
            <div className="mt-4 space-y-2 text-left">
                <div className="flex items-center gap-3"> <PhoneIcon /> <span className="text-gray-600">{doctor.number}</span> </div>
                <div className="flex items-center gap-3"> <EmailIcon /> <span className="text-gray-600">{doctor.email}</span> </div>
            </div>
          </div>
          <div className="flex-grow md:w-2/3">
            <h2 className="text-2xl font-bold text-gray-800 border-b pb-2 mb-4">About Dr. {doctor.name.split(' ').slice(1).join(' ')}</h2>
            <p className="text-gray-700 leading-relaxed">{doctor.description}</p>
            <div className="mt-8">
              <h2 className="text-2xl font-bold text-gray-800 border-b pb-2 mb-4">Weekly Availability</h2>
              {Object.keys(parsedAvailability).length > 0 ? (
                <div className="space-y-3">
                  {Object.entries(parsedAvailability).map(([day, times]) => (
                    <div key={day} className="grid grid-cols-3 gap-4 items-center">
                      <strong className="text-gray-700 col-span-1">{day}:</strong>
