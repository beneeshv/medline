...
export default function DoctorPrescriptions() {
  ...
  const handlePrescriptionSubmit = useCallback(async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/prescriptions/create/', {
        appointment_id: selectedAppointment,
        medications,
        instructions
      });
      setSuccess(response.data.message || 'Prescription sent successfully!');
      setMedications('');
      setInstructions('');
    } catch (err) {
      console.error('Error sending prescription:', err);
      const errorMessage = err.response?.data?.message || 'Failed to send prescription. Please try again.';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white">
      {/* Header */}
      <header className="bg-white shadow-md border-b border-gray-200"> {/* Changed bg-teal-700 to bg-white for black text contrast */}
        <div className="max-w-7xl mx-auto px-4 py-6 flex justify-between items-center">
          <h1 className="text-3xl font-bold text-black">Apollo Hospital</h1> {/* Ensured text-black */}
          {doctor && (
            <div className="flex items-center space-x-4">
              <div className="text-right text-black"> {/* Ensured text-black */}
              ...
              <button
                ...
                // Changed button style for black text contrast: using white background, dark border and black text
                className="bg-gray-100 hover:bg-gray-200 border border-gray-400 text-black px-4 py-2 rounded-md text-sm font-medium transition-colors"
              >
                Logout
              </button>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="bg-teal-100 px-6 py-4 border-b border-teal-200"> {/* Changed bg-teal-600 to bg-teal-100 for black text contrast */}
            <h2 className="text-2xl font-bold text-black">Prescriptions & Bills</h2> {/* Ensured text-black */}
          </div>

          {/* Navigation Tabs */}
          <div className="bg-teal-50 px-6 py-3 flex space-x-6 border-b border-teal-200">
            ...
            {loading ? (
              ...
              <div>
                <div className="mb-6">
                  ...
                  >
                    <option value="" className="text-black">Select an appointment</option>
                    {appointmentOptions}
                  </select>
                </div>
                ...
                {activeTab === 'prescription' ? (
                  <form onSubmit={handlePrescriptionSubmit} className="bg-white rounded-lg p-6 border border-gray-100">
                    <div className="mb-5">
                      ...
                      <textarea id="medications" className="w-full bg-white border border-gray-300 rounded-md py-3 px-4 text-black" rows="4" placeholder="e.g., Paracetamol 500mg - 1 tablet every 6 hours" value={medications} onChange={(e) => setMedications(e.target.value)} required ></textarea>
                    </div>
                    <div className="mb-6">
                      <label className="block text-black text-sm font-bold mb-2" htmlFor="instructions">Instructions</label>
                      <textarea id="instructions" className="w-full bg-white border border-gray-300 rounded-md py-3 px-4 text-black" rows="3" placeholder="e.g., Take after meals, Avoid alcohol" value={instructions} onChange={(e) => setInstructions(e.target.value)} ></textarea>
                    </div>
                    <div className="flex items-center justify-end">
                      <button type="submit" 
                        // Changed button style for black text contrast: using teal background, white text (exception for buttons)
